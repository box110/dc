...
3. Narrate by appending attributed lines with `say.py` — the web app renders
   ...
   `pc`/`npc`/`boss` for speech); write more dialogue, not one big block. Make
   any decision roll with `roll.py` so the players watch the die spin and land
   in the dashboard. The CLI chat is **control-only**: dice results, state
   changes, and a one-line "your move?" prompt — not prose.
...
| `roll.py <cid> '<json>'` | Roll decision dice; animates a spinning die (right type) in the web app + reports HIT/MISS. `{label,notation,adv?,dis?,vs?,kind?}`. |
